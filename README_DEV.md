...
- **LRCLIB lyrics coverage.** The theme categorizer uses LRCLIB for lyrics; not every song has lyrics available there. A song with no lyrics is "uncovered" for theme — it's grouped using the other dimensions, not treated as dissimilar to everything (see [Grouping explained](#4-grouping-explained)). The keywords and language categorizers depend on the same lyrics and are uncovered for the same songs.
